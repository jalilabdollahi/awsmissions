...
aws_local lambda create-function   --function-name mission-function   --runtime python3.6   --role arn:aws:iam::000000000000:role/mission-lambda-role   --handler index.lambda_handler   --zip-file fileb://"$tmpdir/function.zip" >/dev/null 2>&1 || true

echo "Broken function created: mission-function uses runtime python3.6"
