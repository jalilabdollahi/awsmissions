...
aws_local iam delete-role-policy --role-name mission-lambda-role --policy-name mission-lambda-logs >/dev/null 2>&1 || true
aws_local iam delete-role --role-name mission-lambda-role >/dev/null 2>&1 || true

echo "Broken function created: execution role mission-lambda-role no longer exists"
