...
aws_local lambda create-function   --function-name mission-function   --runtime python3.11   --role arn:aws:iam::000000000000:role/mission-lambda-role   --handler index.lambda_handler   --vpc-config SubnetIds=[subnet-12345678],SecurityGroupIds=[]   --zip-file fileb://"$tmpdir/function.zip" >/dev/null 2>&1 || true

echo "Broken function created: VPC config has no security groups"
